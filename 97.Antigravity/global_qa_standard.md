...
5.  **AI 인사이트 검증 (AI Insight Validation)**: 수집된 데이터가 AI에 의해 올바르게 분석되어 의사결정에 유용한 인사이트로 변환되는지 확인합니다. (비용 효율적인 Gemini Free Tier 활용)
6.  **문서화 (Documentation)**: 발견된 문제와 해결 과정을 기록하여 향후 동일한 문제가 발생하지 않도록 방지합니다.
