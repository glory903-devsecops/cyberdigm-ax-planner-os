...
![Intelligence](./docs/images/intelligence.png)
...
- **AI Decision Support**: 수집된 데이터를 Gemini AI가 분석하여 전략적 인사이트 및 액션 아이템 제안 (비용 효율적 구조).
- **Security & Compliance**: ISMS-P 기준을 준수하는 SHA Pinning 공급망 보안 및 Supabase RLS 정책 적용.
...
### 1. Supabase 데이터베이스 구축 (Master Setup)
플랫폼의 모든 기능을 정상적으로 사용하려면 Supabase SQL Editor에서 아래 경로의 스크립트를 실행하여 테이블과 권한을 초기화해야 합니다.
1. [Supabase](https://supabase.com/)에서 새 프로젝트 생성 후 **SQL Editor**로 이동합니다.
2. [`sql/master_setup.sql`](./sql/master_setup.sql) 파일 내용을 복사하여 실행(Run)합니다.

### 2. GitHub Secrets 설정
자동 크롤링 및 AI 분석을 위해 **Settings > Secrets and variables > Actions**에 아래 항목을 등록하십시오.
...
## 🗺️ Data Evidence Expansion Roadmap
AI 의사결정의 신뢰도를 높이기 위해 다음과 같은 고부가가치 데이터 채널을 추가로 통합할 예정입니다.

1. **재무 시그널 (Financial Intelligence)**: DART 공시 연동을 통해 경쟁사의 R&D 투자 규모 및 매출 추이를 분석합니다.
2. **인재 시그널 (Talent Intelligence)**: 주요 채용 플랫폼(사람인, 잡코리아) 크롤링을 통해 경쟁사의 AI 관련 인력 채용 현황 및 기술 스택 변화를 감지합니다.
3. **기술 시그널 (Technical Intelligence)**: 특허청(KIPRIS) 데이터 연동으로 경쟁사의 신규 특허 출원 및 기술 방어 범위를 모니터링합니다.
